...
class TSDF:
    def __init__(self, voxel_size=0.04):
        # Take care of open3d api change from 0.11
        o3d_integration = getattr(o3d, 'pipelines', o3d).integration
            
        self.volume = o3d_integration.ScalableTSDFVolume(
            voxel_length=voxel_size,
            sdf_trunc=voxel_size*3,  # truncation value is set at 3x voxel size
            color_type=o3d_integration.TSDFVolumeColorType.RGB8)
        #   color_type=o3d.integration.TSDFVolumeColorType.NoColor)

    def process_dir(self, pinhole_path):
        pinhole_path = Path(pinhole_path)
        # WARNING: in read_pinhole_camera_trajectory extrinsic gets inverted!
        trajectory = o3d.io.read_pinhole_camera_trajectory(str(pinhole_path / 'odometry.log'))
        intrinsic = np.loadtxt(str(pinhole_path / "calibration.txt"))
        with open(pinhole_path / "rgb.txt") as rf, open(str(pinhole_path / "depth.txt")) as df:
            for i, (rgb_line, depth_line) in enumerate(zip(rf, df)):
                rgb = np.array(o3d.io.read_image(str(pinhole_path / rgb_line.split()[1])))
                depth = np.array(o3d.io.read_image(str(pinhole_path / depth_line.split()[1])))
                self.integrate_frame(rgb, depth, intrinsic, trajectory.parameters[i].extrinsic)
        self.render()

    def integrate_frame(self, rgb, depth, intrinsic, trajectory_extrinsic):
        o3drgb = o3d.geometry.Image((rgb).astype(np.uint8))
        o3ddepth = o3d.geometry.Image((depth).astype(np.uint8))
        h, w = depth.shape[:2]
        intrinsic = o3d.camera.PinholeCameraIntrinsic(w, h, *intrinsic[:4])
        rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(
            o3drgb, o3ddepth,
            depth_scale=DEPTH_SCALING_FACTOR,
            depth_trunc=7.8, convert_rgb_to_intensity=False)
        self.volume.integrate(rgbd, intrinsic, trajectory_extrinsic)
...
def main(pinhole_path: str, voxel_size=0.04):
    ts = TSDF(voxel_size)
    ts.process_dir(pinhole_path)
